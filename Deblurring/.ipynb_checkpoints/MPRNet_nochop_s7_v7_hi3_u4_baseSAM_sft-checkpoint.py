...
#             print('enc+dec:',out.shape,enc.shape,dec.shape)
            ...
            out = out*F.sigmoid(self.phi(skip_)) + self.gamma(skip_)
        if self.downsample:
...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=96, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=4, bias=False, depth=5):
