...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3, reduction=4, bias=False):
        ...
        self.stage2_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4, csff=False)
        self.stage2_decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4)

        self.stage3_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4, csff=False)
        self.stage3_decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4)

        self.stage4_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4, csff=False)
        self.stage4_decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4)
        
        self.stage5_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4, csff=False)
        self.stage5_decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4)
        
        self.stage6_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4, csff=False)
        self.stage6_decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4)
        
        self.t1=conv(n_feat, 3, kernel_size, bias=bias)
        self.t2=conv(n_feat, 3, kernel_size, bias=bias)
        self.t3=conv(n_feat, 3, kernel_size, bias=bias)
        self.t4=conv(n_feat, 3, kernel_size, bias=bias)
        self.t5=conv(n_feat, 3, kernel_size, bias=bias)
        self.t6=conv(n_feat, 3, kernel_size, bias=bias)
...
#         x1_img = img
        ...
        stage1_img = self.t1(res1[-1])+x1_img
...
#         x2_cat = self.concat12(torch.cat([x2, x2_samfeats], 1))
        ...
        feat2,feat_fin2 = self.stage2_encoder(x2)

        ## Pass features through Decoder of Stage 2
        res2 = self.stage2_decoder(feat_fin2,feat2)

        ## Apply SAM
        stage2_img = self.t2(res2[-1])+x2_img
...
#         x3_cat = self.concat23(torch.cat([x3, x3_samfeats], 1))
        ...
        ## Process features of both patches with Encoder of Stage 2
        feat3,feat_fin3 = self.stage3_encoder(x3)

        ## Pass features through Decoder of Stage 2
        res3 = self.stage3_decoder(feat_fin3,feat3)

        ## Apply SAM
        stage3_img = self.t3(res3[-1])+x3_img
...
#         x4_cat = self.concat34(torch.cat([x4, x4_samfeats], 1))
        ## Process features of both patches with Encoder of Stage 2
        feat4,feat_fin4 = self.stage4_encoder(x4)
        ## Pass features through Decoder of Stage 2
        res4 = self.stage4_decoder(feat_fin4,feat4)
        ## Apply SAM
        stage4_img = self.t4(res4[-1])+x4_img
...
#         x5_cat = self.concat45(torch.cat([x5, x5_samfeats], 1))
        ## Process features of both patches with Encoder of Stage 2
        feat5,feat_fin5 = self.stage5_encoder(x5)
        ## Pass features through Decoder of Stage 2
        res5 = self.stage5_decoder(feat_fin5,feat5)
        ## Apply SAM
        stage5_img = self.t5(res5[-1])+x5_img
#         x6_samfeats, stage5_img = self.sam56(res5[-1], x5_img)
        ...
        ## Concatenate SAM features of Stage 1 with shallow features of Stage 2
#         x6_cat = self.concat56(torch.cat([x6, x6_samfeats], 1))
        ## Process features of both patches with Encoder of Stage 2
        feat6,feat_fin6 = self.stage6_encoder(x6)
        ## Pass features through Decoder of Stage 2
        res6 = self.stage6_decoder(feat_fin6,feat6)
        ## Apply SAM
        stage6_img = self.t6(res6[-1])+x6_img
...
#         x7_cat = self.concat67(torch.cat([x7, x7_samfeats], 1))
        stage7_img = self.tail(x7)+ img
