...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3, reduction=4, bias=False):
        ...
        self.phi_1 = nn.Parameter(init.xavier_normal_(torch.Tensor(3, 3 , 3, 3)))
        ...
        self.phi_2 = nn.Parameter(init.xavier_normal_(torch.Tensor(3, 3 , 3, 3)))
    ...
    def forward(self, x3_img):
        ...
        top2_img = x3_img[:,:,0:int(H/2),:]
        bot2_img = x3_img[:,:,int(H/2):H,:]
        x1ltop_img = top2_img[:,:,:,0:int(W/2)]
        x1rtop_img = top2_img[:,:,:,int(W/2):W]
        x1lbot_img = bot2_img[:,:,:,0:int(W/2)]
        x1rbot_img = bot2_img[:,:,:,int(W/2):W]
        ...
        x2top_samfeats, stage1_img_top = self.sam12(res1_top[0], top2_img)
        x2bot_samfeats, stage1_img_bot = self.sam12(res1_bot[0], bot2_img)
        ...
        phixsy_2 = F.conv2d(stage1_img, self.phi_1, padding=1) - x3_img
        x2_img = stage1_img - self.r1*F.conv2d(phixsy_2, self.phi_1.transpose(3,2).contiguous(), padding=1)#x3_img*self.r1+(1-self.r1)*stage1_img
        x2top_img = x2_img[:,:,0:int(H/2),:]
        x2bot_img = x2_img[:,:,int(H/2):H,:]
        ## Compute Shallow Features
        ...
        ## Apply SAM
        x3_samfeats, stage2_img = self.sam23(res2[0], x2_img)
        ...
        ##-------------------------------------------
        ## Compute Shallow Features
#         x3_img = x3_img*self.r2+(1-self.r2)*stage2_img
        phixsy_3 = F.conv2d(stage2_img, self.phi_2, padding=1) - x3_img
        x3_img = stage2_img - self.r2*F.conv2d(phixsy_3, self.phi_2.transpose(3,2).contiguous(), padding=1)
