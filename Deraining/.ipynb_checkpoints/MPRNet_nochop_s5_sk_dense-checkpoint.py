...
class Encoder(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, act, bias, scale_unetfeats, csff):
        ...
        if csff:
            self.sk1 = SKConv(n_feat, 2, 2)
            self.sk2 = SKConv(n_feat + scale_unetfeats, 2, 2)
            self.sk3 = SKConv(n_feat + scale_unetfeats * 2, 2, 2)
    ...
    def forward(self, x, encoder_outs=None, decoder_outs=None):
        ...
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc1 = enc1 + self.sk1(encoder_outs[0],decoder_outs[0])

        x = self.down12(enc1)

        enc2 = self.encoder_level2(x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc2 = enc2 + self.sk2(encoder_outs[1],decoder_outs[1])

        x = self.down23(enc2)

        enc3 = self.encoder_level3(x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc3 = enc3 + self.sk3(encoder_outs[2],decoder_outs[2])
...
class MPRNet(nn.Module):
    ...
    def forward(self, x3_img):
        ...
        for i in range(len(feat1)):
            feat2[i] = feat2[i]+feat1[i]
            res2[i] = res2[i]+res1[i]
        ...
        for i in range(len(feat2)):
            feat3[i] = feat3[i]+feat2[i]
            res3[i] = res3[i]+res2[i]
        ...
        for i in range(len(feat3)):
            feat4[i] = feat4[i]+feat3[i]
            res4[i] = res4[i]+res3[i]
