...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=40, scale_unetfeats=20, scale_orsnetfeats=16, num_cab=8, kernel_size=3, reduction=4, bias=False):
        ...
        self.shallow_feat7 = nn.Sequential(conv(3, n_feat, kernel_size, bias=bias),
                                           CAB(n_feat, kernel_size, reduction, bias=bias, act=act))
        self.shallow_feat8 = nn.Sequential(conv(3, n_feat, kernel_size, bias=bias),
                                           CAB(n_feat, kernel_size, reduction, bias=bias, act=act))
        self.shallow_feat9 = nn.Sequential(conv(3, n_feat, kernel_size, bias=bias),
                                           CAB(n_feat, kernel_size, reduction, bias=bias, act=act))
        ...
        self.stage7_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4, csff=True)
        self.stage7_decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4)
        
        self.stage8_encoder = Encoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4, csff=True)
        self.stage8_decoder = Decoder(n_feat, kernel_size, reduction, act, bias, scale_unetfeats,depth=4)
        ...
        self.sam78 = SAM(n_feat, kernel_size=1, bias=bias)
        self.merge78=mergeblock(n_feat,3,True)
        self.sam89 = SAM(n_feat, kernel_size=1, bias=bias)
        self.merge89=mergeblock(n_feat,3,True)
        ...
        self.phi_7 = ResBlock(default_conv,3,3)
        self.phit_7 = ResBlock(default_conv,3,3)
        self.phi_8 = ResBlock(default_conv,3,3)
        self.phit_8 = ResBlock(default_conv,3,3)
        ...
        self.r7 = nn.Parameter(torch.Tensor([0.5]))
        self.r8 = nn.Parameter(torch.Tensor([0.5]))
...
#         x3_img = img * self.r2 + (1 - self.r2) * stage2_img
        ...
        ## Concatenate SAM features of Stage 1 with shallow features of Stage 2
        x7_cat = self.merge67(x7, x7_samfeats)#self.concat56(torch.cat([x6, x6_samfeats], 1))
        ## Process features of both patches with Encoder of Stage 2
        feat7,feat_fin7 = self.stage7_encoder(x7_cat, feat6, res6)
        ## Pass features through Decoder of Stage 2
        res7 = self.stage7_decoder(feat_fin7,feat7)
        ## Apply SAM
        x8_samfeats, stage7_img = self.sam78(res7[-1], x7_img)
        
        ##-------------------------------------------
        ##-------------- Stage 8---------------------
        ##-------------------------------------------
        ## Compute Shallow Features
        phixsy_8 = self.phi_7(stage7_img) - img
        x8_img = stage7_img - self.r7*self.phit_7(phixsy_8)
        x8 = self.shallow_feat8(x8_img)
        ## Concatenate SAM features of Stage 1 with shallow features of Stage 2
        x8_cat = self.merge78(x8, x8_samfeats)#self.concat56(torch.cat([x6, x6_samfeats], 1))
        ## Process features of both patches with Encoder of Stage 2
        feat8,feat_fin8 = self.stage8_encoder(x8_cat, feat7, res7)
        ## Pass features through Decoder of Stage 2
        res8 = self.stage8_decoder(feat_fin8,feat8)
        ## Apply SAM
        x9_samfeats, stage8_img = self.sam89(res8[-1], x8_img)

#         ##-------------------------------------------
#         ##-------------- Stage 9---------------------
#         ##-------------------------------------------
#         ## Compute Shallow Features
        phixsy_9 = self.phi_8(stage8_img) - img
        x9_img = stage8_img - self.r8*self.phit_8(phixsy_9)
        x9 = self.shallow_feat9(x9_img)
#         ## Concatenate SAM features of Stage 2 with shallow features of Stage 3
        x9_cat = self.merge89(x9, x9_samfeats)#self.concat67(torch.cat([x7, x7_samfeats], 1))
        stage9_img = self.tail(x9_cat)+ img

        return [stage9_img, stage8_img,stage7_img,stage6_img,stage5_img,stage4_img, stage3_img, stage2_img, stage1_img]#[stage5_img + img, stage4_img, stage3_img, stage2_img, stage1_img]
